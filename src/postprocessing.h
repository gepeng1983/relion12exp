...
public:
	...
	// Whether to load helicised volume
	bool helix;
...
public:
	...
	void divideByMtf(MultidimArray<std::complex<double> > &FT);

	// Make a Guinier plot from the Fourier transform of an image
	void makeGuinierPlot(MultidimArray<std::complex<double> > &FT, std::vector<fit_point2D> &guinier);

	// Fit straight line through Guinier plot
	void fitStraightLine(std::vector<fit_point2D> &guinier, double &slope, double &intercept, double &corr_coeff);

	// Apply sqrt(2FSC/(FSC=1)) weighting prior to B-factor sharpening
	void applyFscWeighting(MultidimArray<std::complex<double> > &FT, MultidimArray<double> my_fsc);

	// Apply B-factor sharpening
	void applyBFactor(MultidimArray<std::complex<double> > &FT, double bfactor);

	// Apply a soft-edge filter in Fourier space
	void lowPassFilterMap(MultidimArray<std::complex<double> > &FT, double low_pass);
