...
void BackProjector::backproject(const MultidimArray<std::complex<double> > &f2d,
		...
		for (int x=first_x; x <= r_max; x++)
		{
			...
			if (my_weight > 0.)
			{
				...
				else if (interpolator == NEAREST_NEIGHBOUR )
				{
					...
					if (x0 < 0)
					{
						A3D_ELEM(data, -z0, -y0, -x0) += std::conj(my_val);
...
void BackProjector::backrotate2D(const MultidimArray<std::complex<double> > &f2d,
		                         const Matrix2D<double> &A, bool inv,
		                         const MultidimArray<double> *Mweight)
{
		...
		for (int x=first_x; x <= r_max; x++)
		{
			...
			if (my_weight > 0.)
			{
				...
				else if (interpolator == NEAREST_NEIGHBOUR )
				{
					...
					if (x0 < 0)
					{
						A2D_ELEM(data, -y0, -x0) += std::conj(my_val);
...
void BackProjector::getLowResDataAndWeight(MultidimArray<std::complex<double> > &lowres_data, MultidimArray<double> &lowres_weight,
		int lowres_r_max)
{
...
void BackProjector::setLowResDataAndWeight(MultidimArray<std::complex<double> > &lowres_data, MultidimArray<double> &lowres_weight,
		int lowres_r_max)
{
...
void BackProjector::getDownsampledAverage(MultidimArray<std::complex<double> > &avg)
{
...
void BackProjector::calculateDownSampledFourierShellCorrelation(MultidimArray<std::complex<double> > &avg1,
																MultidimArray<std::complex<double> > &avg2,
																MultidimArray<double> &fsc)
{
    ...
    FOR_ALL_ELEMENTS_IN_ARRAY3D(avg1)
    {
        ...
        if (R > r_max)
            continue;
        int idx=ROUND(R);
        std::complex<double> z1=A3D_ELEM(avg1, k, i, j);
        std::complex<double> z2=A3D_ELEM(avg2, k, i, j);
        double absz1=abs(z1);
        double absz2=abs(z2);
...

{
	...
	MultidimArray<std::complex<double> > Fconv;
...
//#define RECONSTRUCT_CONVOLUTE_BLOB
	...
	MultidimArray<std::complex<double> > Ftmp;
...
void BackProjector::enforceHermitianSymmetry(MultidimArray<std::complex<double> > &my_data,
											 MultidimArray<double> &my_weight)
{
	...
	for (int iz = STARTINGZ(my_data); iz <=FINISHINGZ(my_data); iz++)
	{
		...
		for (int iy = starty; iy <= FINISHINGY(my_data); iy++)
		{
			...
			std::complex<double> fsum = (A3D_ELEM(my_data, iz, iy, 0) + std::conj(A3D_ELEM(my_data, -iz, -iy, 0)));
			A3D_ELEM(my_data, iz, iy, 0) = fsum;
			A3D_ELEM(my_data, -iz, -iy, 0) = std::conj(fsum);
...
void BackProjector::symmetrise(MultidimArray<std::complex<double> > &my_data,
...
//#define DEBUG_SYMM
	...
	if (SL.SymsNo() > 0 && ref_dim == 3)
	{
		...
		MultidimArray<std::complex<double> > sum_data;
        double x, y, z, fx, fy, fz, xp, yp, zp, r2;
        bool is_neg_x;
        int x0, x1, y0, y1, z0, z1;
    	std::complex<double> d000, d001, d010, d011, d100, d101, d110, d111;
    	std::complex<double> dx00, dx01, dx10, dx11, dxy0, dxy1;
    	double dd000, dd001, dd010, dd011, dd100, dd101, dd110, dd111;
	    ...
	    for (int isym = 0; isym < SL.SymsNo(); isym++)
	    {
	        ...
	        FOR_ALL_ELEMENTS_IN_ARRAY3D(sum_weight)
	        {
	        	...
	        	if (r2 <= my_rmax2)
	        	{
					...
					if (is_neg_x)
						A3D_ELEM(sum_data, k, i, j) += std::conj(LIN_INTERP(fz, dxy0, dxy1));
					else
...
void BackProjector::windowToOridimRealSpace(FourierTransformer &transformer, MultidimArray<std::complex<double> > &Fin, MultidimArray<double> &Mout, int nr_threads)
{
