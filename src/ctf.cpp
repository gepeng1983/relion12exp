...
/* Read -------------------------------------------------------------------- */
void CTF::read(MetaDataTable &MD1, MetaDataTable &MD2)
{

	if (!MD1.getValue(EMDL_CTF_VOLTAGE, kV))
		if (!MD2.getValue(EMDL_CTF_VOLTAGE, kV))
			kV=200;

	if (!MD1.getValue(EMDL_CTF_DEFOCUSU, DeltafU))
		if (!MD2.getValue(EMDL_CTF_DEFOCUSU, DeltafU))
			DeltafU=0;

	if (!MD1.getValue(EMDL_CTF_DEFOCUSV, DeltafV))
		if (!MD2.getValue(EMDL_CTF_DEFOCUSV, DeltafV))
			DeltafV=DeltafU;

	if (!MD1.getValue(EMDL_CTF_DEFOCUS_ANGLE, azimuthal_angle))
		if (!MD2.getValue(EMDL_CTF_DEFOCUS_ANGLE, azimuthal_angle))
			azimuthal_angle=0;

	if (!MD1.getValue(EMDL_CTF_CS, Cs))
		if (!MD2.getValue(EMDL_CTF_CS, Cs))
			Cs=0;

	if (!MD1.getValue(EMDL_CTF_BFACTOR, Bfac))
		if (!MD2.getValue(EMDL_CTF_BFACTOR, Bfac))
			Bfac=0;

	if (!MD1.getValue(EMDL_CTF_Q0, Q0))
		if (!MD2.getValue(EMDL_CTF_Q0, Q0))
			Q0=0;

	initialise();
}
void CTF::setValues(double _defU, double _defV, double _defAng, double _voltage, double _Cs, double _Q0, double _Bfac)
{
	kV              = _voltage;
	DeltafU         = _defU;
	DeltafV         = _defV;
