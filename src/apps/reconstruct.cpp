...
class reconstruct_parameters
{
	...
	MetaDataTable DF;
	int r_max, r_min_nn, blob_order, padding_factor, ref_dim, oridim, interpolator, iter, nr_threads, debug_ori_size, debug_size;
	double blob_radius, blob_alpha, angular_error, shift_error, angpix, maxres;
	bool do_ctf, ctf_phase_flipped, only_flip_phases, intact_ctf_first_peak, do_fom_weighting;
	// I/O Parser
	IOParser parser;
	...
	void reconstruct()
	{
		...
		Matrix2D<double> A3D;
		MultidimArray<std::complex<double> > Faux, F2D, Fsub;
...
//#define DEBUG_WW
		...
		FOR_ALL_DIRECT_ELEMENTS_IN_MULTIDIMARRAY(Imagn())
		{
			...
			DIRECT_MULTIDIM_ELEM(backprojectort.data, n) = (std::complex<double>)(realval, imagval);
		}
...
//#define DEBUG_RECONSTRUCT_ONLY
					...
					//if (part_id == my_first_particle_id)
					{
						...
						MultidimArray<std::complex<double> > Faux = F2D;
						FourierTransformer transformer;
