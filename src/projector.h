...
public:
    ...
    MultidimArray<std::complex<double> > data;
...
public:
   ...
   /** Constructor with parameters
     ...
     * @endcode
     */
    Projector(int _ori_size, int _interpolator = NEAREST_NEIGHBOUR, int _padding_factor_3d = 3, int _r_min_nn = 10)
    {
    ...
    *
    */
   void computeFourierTransformMap(MultidimArray<double> &vol_in, MultidimArray<double> &power_spectrum, int current_size = -1, int nr_threads = 1);
	...
	*/
	void get2DFourierTransform(MultidimArray<std::complex<double> > &img_out, Matrix2D<double> &A, bool inv)
	{
		switch (ref_dim)
		{
		case 2:
	...
	*/
	void project(MultidimArray<std::complex<double> > &img_out, Matrix2D<double> &A, bool inv);

	/*
	* Get an in-plane rotated version of the 2D map (mere interpolation)
	*/
	void rotate2D(MultidimArray<std::complex<double> > &img_out, Matrix2D<double> &A, bool inv);
