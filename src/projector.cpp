...
void Projector::computeFourierTransformMap(MultidimArray<double> &vol_in, MultidimArray<double> &power_spectrum, int current_size, int nr_threads)
{
...
void Projector::project(MultidimArray<std::complex<double> > &f2d, Matrix2D<double> &A, bool inv)
{
	...
	for (int i=0; i < YSIZE(f2d); i++)
	{
		...
		for (int x=0; x <= my_r_max; x++)
		{
			...
			if (interpolator == TRILINEAR || r2 < min_r2_nn)
			{
				...
				if (is_neg_x)
					DIRECT_A2D_ELEM(f2d, i, x) = std::conj(DIRECT_A2D_ELEM(f2d, i, x));
			...
			else if (interpolator == NEAREST_NEIGHBOUR )
			{
				...
				z0 = ROUND(zp);
				if (x0 < 0)
					DIRECT_A2D_ELEM(f2d, i, x) = std::conj(A3D_ELEM(data, -z0, -y0, -x0));
				else
...
void Projector::rotate2D(MultidimArray<std::complex<double> > &f2d, Matrix2D<double> &A, bool inv)
{
	double fx, fy, xp, yp;
	int x0, x1, y0, y1, y, y2, r2;
	bool is_neg_x;
	std::complex<double> d00, d01, d10, d11, dx0, dx1;
	Matrix2D<double> Ainv;
	...
	for (int i=0; i < YSIZE(f2d); i++)
	{
		...
		for (int x=0; x <= my_r_max; x++)
		{
			...
			else if (interpolator == NEAREST_NEIGHBOUR )
			{
				...
				y0 = ROUND(yp);
				if (x0 < 0)
					DIRECT_A2D_ELEM(f2d, i, x) = std::conj(A2D_ELEM(data, -y0, -x0));
				else
