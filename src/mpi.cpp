...
int MpiNode::relion_MPI_Send(void *buf, long int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm)
{
...
#define USE_SEGMENTED_SEND
...
#else
#ifdef USE_SEGMENTED_SEND
	long int size;
	int isize;
	MPI_Type_size(datatype, &isize);
	size=isize;
	if (count*size > (long int) 2*1024*1024*1024)
	{
		long int packet = (long int)1*1024*1024*1024/size;
		long int i;
		for (i=0; i<count-packet; i+=packet)
		{
			result=MPI_Send(buf+i*size, packet, datatype, dest, tag, comm);
			if (result != MPI_SUCCESS)
			{
				report_MPI_ERROR(result);
			}
		}
		result=MPI_Send(buf+i*size, count-i, datatype, dest, tag, comm);
		if (result != MPI_SUCCESS)
		{
			report_MPI_ERROR(result);
		}
	...
	if (count > 100)
	{
		...
		char *membuff = (char *) malloc(size + MPI_BSEND_OVERHEAD);

		// Allocate memory for the package to be sent
		int attach_result = MPI_Buffer_attach( membuff, size + MPI_BSEND_OVERHEAD );
		if (attach_result != MPI_SUCCESS)
		{
			report_MPI_ERROR(attach_result);
		}
		...
		{
			report_MPI_ERROR(deattach_result);
		}
		free(membuff);
	}
	else
...
int MpiNode::relion_MPI_Recv(void *buf, long int count, MPI_Datatype datatype, int source, int tag, MPI_Comm comm, MPI_Status &status)
{
	int result;
	MPI_Request request;
	...
	if (count*size > (long int)2*1024*1024*1024)
	{
		...
		for (i=0; i<count-packet; i+=packet)
		{
			result=MPI_Recv(buf+i*size, packet, datatype, source, tag, comm, &status);
			if (result != MPI_SUCCESS)
			{
				report_MPI_ERROR(result);
			}
		}
		result=MPI_Recv(buf+i*size, count-i, datatype, source, tag, comm, &status);
		if (result != MPI_SUCCESS)
		{
			report_MPI_ERROR(result);
		}
	...
	{
		result_irecv = MPI_Irecv(buf, count, datatype, source, tag, comm, &request);
...
int MpiNode::relion_MPI_Bcast(void *buffer, long int count, MPI_Datatype datatype, int root, MPI_Comm comm)
{
	...
	if (count*size > (long int)2*1024*1024*1024)
	{
		...
		for (i=0; i<count-packet; i+=packet)
		{
			result = MPI_Bcast(buffer+i*size, packet, datatype, root, comm);
			if (result != MPI_SUCCESS)
			{
				report_MPI_ERROR(result);
			}
		}
		result = MPI_Bcast(buffer+i*size, count-i, datatype, root, comm);	
		if (result != MPI_SUCCESS)
		{
			report_MPI_ERROR(result);
		}
...
void printMpiNodesMachineNames(MpiNode &node, int nthreads)
{
	...
	if (node.isMaster())
	{
        ...
        std::cout << " + Master  (0) runs on host            = ";
        std::cout.flush();
