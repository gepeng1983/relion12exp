...
public:
	...
	BackProjector(int _ori_size, int _ref_dim, FileName fn_sym,
			      int _interpolator = NEAREST_NEIGHBOUR, int _padding_factor_3d = 3, int _r_min_nn = 10,
	...
	*/
	void set2DFourierTransform(const MultidimArray<std::complex<double> > &img_in,
	...
	*/
	void backrotate2D(const MultidimArray<std::complex<double> > &img_in,
			          const Matrix2D<double> &A, bool inv,
			          const MultidimArray<double> *Mweight = NULL);
	...
	* If a exp_Mweight is given, rather than adding 1 to all relevant pixels in the weight array, we use exp_Mweight
	*/
	void backproject(const MultidimArray<std::complex<double> > &img_in,
			         const Matrix2D<double> &A, bool inv,
			         const MultidimArray<double> *Mweight = NULL);
	 ...
	 */
	void getLowResDataAndWeight(MultidimArray<std::complex<double> > &lowres_data, MultidimArray<double> &lowres_weight,
			int lowres_r_max);
	 ...
	 * (to be joined together for two independent halves in order to force convergence in the same orientation)
	 */
	void setLowResDataAndWeight(MultidimArray<std::complex<double> > &lowres_data, MultidimArray<double> &lowres_weight,
			int lowres_r_max);
	 ...
	 */
	void getDownsampledAverage(MultidimArray<std::complex<double> > &avg);

	/*
	 * From two of the straightforward downsampled averages, calculate an FSC curve
	 */
	void calculateDownSampledFourierShellCorrelation(MultidimArray<std::complex<double> > &avg1,
    ...
    */
	void enforceHermitianSymmetry(MultidimArray<std::complex<double> > &mydata,
								  MultidimArray<double> &myweight);

	/* Applies the symmetry from the SymList object to the weight and the data array
	 */
	void symmetrise(MultidimArray<std::complex<double> > &mydata,
   ...
   /* Convolute in Fourier-space with the blob by multiplication in real-space
	 ...
	 */
	void windowToOridimRealSpace(FourierTransformer &transformer, MultidimArray<std::complex<double> > &Fin, MultidimArray<double> &Mout, int nr_threads);
